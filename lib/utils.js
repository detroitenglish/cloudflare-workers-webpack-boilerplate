...
let isExample, useColors, useEmoji, metadataPath
...
exports.bootstrap = function(envPath = `${process.cwd()}/.env`, fixture) {
  ...
  for (let k in envConfig) {
    if (envConfig[k] != "") process.env[k] = envConfig[k] // eslint-disable-line
  }

  isExample = !!process.env.EXAMPLE_WORKER
  useColors = !process.env.NO_COLORS
  useEmoji = !process.env.NO_EMOJI
  metadataPath = isExample
    ? path.resolve('./src/example.metadata.json')
  ...
  if (!fixture) {
    ...
    const workerFile = `${process.cwd()}/test/fixtures/js/${fixture}.test.js`
    let exists = fs.existsSync(workerFile)
    entry = exists
      ? workerFile
      : `${process.cwd()}/test/fixtures/js/example.test.js`
  }
